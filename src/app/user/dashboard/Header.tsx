...
const Header: React.FC<HeaderProps> = ({ searchQuery, setSearchQuery, handleSearch, isAuthenticated, userEmail, userImage, onOpenAuth, onLogout }) => {
    ...
    return (
        <>
                            ...
                            {isDropdownOpen && (
                                <div style={{
                                    ...
                                    <button
                                        ...
                                        style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'none', border: 'none', color: '#A7F3D0', padding: '8px', textAlign: 'left', cursor: 'pointer', borderRadius: '8px', width: '100%', fontSize: '12px', fontWeight: 600 }}
                                    >
                                        <KeyRound size={14} />
                                        Configure AI key
                                    </button>
            ...
            {isApiKeyModalOpen && <ApiSaveModal onClose={() => setIsApiKeyModalOpen(false)} />}
        </>
    );
};
